...
| **问题 1：双 nfe + 变慢** | 非 VERL 重复调用；是 SGLang 内 **多轮 DLLM staging**（KV/token budget），每轮 append 一个 nfe。 |
...
1. Dream：`needs_full_prefill=True`，不宜复用 prefix KV。
2. 每个 SGLang scheduler round 结束会 `append` 一轮 `nfe`（`maybe_collect_customized_info`）。
3. `add_dllm_staging_req` 因 **KV / `max_num_batched_tokens` / `mem_fraction_static`** 截断时，一条请求拆成多轮 forward → `meta_info["nfe"] = [201, 197]`。
...
| 典型总长 | prompt ~442 + response 512 |

### 建议排查

1. 看 rollout 日志：`format_nfe_for_log` 会打 `nfe_rounds` / `nfe_total`（`dream_rollout_debug.py`）。
2. 对照实验：`enable_chunked_prefill=False`、提高 `max_num_batched_tokens` 或（仅 SGLang 独占 GPU 时）略增 `mem_fraction_static`。
3. 固定一条长 prompt 在 **训练同款** SGLang Engine 上离线 generate，数 scheduler 轮数。
