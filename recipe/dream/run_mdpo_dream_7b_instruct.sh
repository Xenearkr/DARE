...
sample_train_steps=16  # Number of diffusion steps to sample for training
mdpo_epsilon=0.2       # PPO clip range
mdpo_beta=0.02         # KL coefficient (0 to disable ref model)
conf_alg="entropy"     # Confidence algorithm
...
exp_name="${baseline}-bsz${batch_size}-n${n_rollout}-prompt${max_prompt_length}-response${max_response_length}-step${num_diffusion_steps}-lr${lr}-temp${train_temperature}-mdpo_k${sample_train_steps}-gpu${n_gpus_per_node}-${timestamp}"
...
python3 -m verl.trainer.dllm_main_ppo \
    ...
    +actor_rollout_ref.actor.sample_train_steps=$sample_train_steps \
    +actor_rollout_ref.actor.mdpo_epsilon=$mdpo_epsilon \
    +actor_rollout_ref.actor.mdpo_beta=$mdpo_beta \
    +actor_rollout_ref.actor.conf_alg=$conf_alg \
    ...
    +actor_rollout_ref.rollout.conf_alg=$conf_alg \
    +actor_rollout_ref.rollout.rcr=False \
