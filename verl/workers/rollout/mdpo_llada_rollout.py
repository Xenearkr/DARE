...
"""
MDPO-specific rollout for LLaDA that collects the full diffusion trajectory.
...
Uses batchified (non-packed) generation following the MDPO source code:
  mdpo/src/mdlm_generation_utils.py::diffusion_generate
"""
...
from verl.workers.rollout.generate import get_num_transfer_tokens
...
@torch.no_grad()
def mdpo_diffusion_generate_llada(model, prompt, mask_id, prompt_mask=None,
                                   steps=64, gen_length=128, block_length=32,
                                   temperature=0., conf_alg='random', rcr=False,
                                   top_p=None, top_k=None):
    """
    MDPO diffusion generation for LLaDA, closely following the MDPO source code.
    Uses standard HuggingFace model call with attention_mask (no packed sequences).

    Reference: mdpo/src/mdlm_generation_utils.py::diffusion_generate
    ...
    Args:
        ...
        block_length: Block length for block-wise generation.
        temperature: Sampling temperature.
        conf_alg: Confidence algorithm ("random", "llada", "entropy", "topk_margin").
        rcr: Running Confidence Remasking.
        ...
        for num_block in range(num_blocks):
            ...
            for i in range(steps_per_block):
                ...
                if conf_alg == 'random':
                    ...
                    # "llada" or default: use token confidence for remasking
                    remasking_confidence = confidence.clone()

                # Ensure we don't process tokens beyond the current block
                remasking_confidence[:, end_idx:] = -np.inf
                ...
                for j in range(batch_size):
                    ...
                    if rcr:
                        total_remaining = num_transfer_tokens[j, i:].sum().item()
                        _, select_indices = torch.topk(remasking_confidence[j], k=total_remaining)
                        x[j, select_indices] = x0[j, select_indices]
                        overtime_confidence[j, select_indices] = remasking_confidence[j, select_indices].clone()

                        if i != (steps_per_block - 1):
                            overtime_conf_wo_zeros = torch.where(
                                overtime_confidence[j] == 0.0, 1.0, overtime_confidence[j])
                        ...
                        if num_tokens > 0:
                            _, select_indices = torch.topk(remasking_confidence[j], k=num_tokens)
                            x[j, select_indices] = x0[j, select_indices]

                if rcr:
                    intermediate_confidence.append(overtime_confidence[:, -gen_length:].clone())
...
class MDPORollout(BaseRollout):
    def __init__(self, module: nn.Module, config, tokenizer):
        """MDPO rollout for LLaDA that collects full diffusion trajectories."""
        super().__init__()
        self.config = config
        self.module = module
        ...
        self.conf_alg = config.get("conf_alg", "llada")
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        """
        Generate sequences using MDPO diffusion process, collecting the full trajectory.
        ...
        for sample_idx in range(total_batch_size):
            ...
            final_resp, inter_results, inter_conf, inter_inputs = mdpo_diffusion_generate_llada(
                model=self.module,
                prompt=prompt_i,
        ...
        print(f"[RANK{dist.get_rank()}] MDPO generate_sequences total time: {total_time:.2f}s, "
