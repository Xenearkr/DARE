...
"""
In single GPU rollout, sequences are generated directly by sampling from model.
...
from .rollout_utils import execute_cjdream_generation
...
class DLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        """
        Generate sequences using diffusion LLM (Dream style)
        1. For each prompt, generate n_rollout responses y
        2. For each y, perform multiple random masks and regenerate, then use ELBO to approximate loglikelihood of y
        """
        # Start timer
        ...
        gen_kwargs = {
            ...
            "do_sample": self.val_kwargs.get("do_sample", True) if is_validate else True,
        ...
        MAX_MODEL_LENGTH = self.config.max_num_batched_tokens
        total_batch_size = batch_size * n_rollout

        # Dream uses standard batch format, no packing needed
        responses, reversed_traj_unmask_positions, full_input_ids, attention_mask, answers = execute_cjdream_generation(
