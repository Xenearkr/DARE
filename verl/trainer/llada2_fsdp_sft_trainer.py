...
"""
FSDP SFT Trainer for LLaDA2.0-mini (MoE model)
Adapted from llada_fsdp_sft_trainer.py with MoE-specific modifications.

LLaDA2.0-mini Model Configuration:
- Architecture: MoE with 256 experts, 8 experts per token, 1 shared expert
...
class FSDPLLada2SFTTrainer:
    """
    FSDP SFT Trainer for LLaDA2.0-mini (MoE model).

    Key differences from LLaDA trainer:
    - MoE model with 256 experts
    - Different token IDs (mask: 156895, pad: 156892)
    ...
    def __init__(self, config, device_mesh: DeviceMesh, ulysses_device_mesh: DeviceMesh, tokenizer, train_dataset: Dataset, val_dataset: Dataset):
        ...
        if self.device_mesh.get_rank() == 0:
            ...
            if self.use_remove_padding:
                print(
                    "LLaDA2 SFT keeps the block-mask Ulysses path for correctness; "
                    "generic packed remove-padding kernels are not available for this model."
                )
    ...
    def _resolve_total_training_steps(self) -> int:
        if self.config.trainer.total_training_steps is not None:
            return self.config.trainer.total_training_steps
        return len(self.train_dataloader) * self.config.trainer.total_epochs
    ...
    def _build_model_optimizer(self):
        ...
        if self.device_mesh.get_rank() == 0:
            print(f"LLaDA2 model detected: MoE={is_moe}")
            if is_moe:
        ...
        self.total_steps = self._resolve_total_training_steps()
...
def main(config):
    ...
    sp_size = getattr(config, "ulysses_sequence_parallel_size", 1)
    assert sp_size >= 1, f"ulysses_sequence_parallel_size must be >= 1, got {sp_size}"
    assert world_size % sp_size == 0, (
        f"world_size ({world_size}) must be divisible by ulysses_sequence_parallel_size ({sp_size})"
    )

    device_mesh = init_device_mesh(device_type=device_name, mesh_shape=(world_size,), mesh_dim_names=("fsdp",))
    dp_size = world_size // sp_size
    ulysses_device_mesh = init_device_mesh(device_type=device_name, mesh_shape=(dp_size, sp_size), mesh_dim_names=("dp", "sp"))
    ...
    trainer = FSDPLLada2SFTTrainer(config=config, device_mesh=device_mesh, ulysses_device_mesh=ulysses_device_mesh, tokenizer=tokenizer, train_dataset=train_dataset, val_dataset=val_dataset)
