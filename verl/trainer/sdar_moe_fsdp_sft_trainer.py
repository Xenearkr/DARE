...
"""
FSDP SFT Trainer for SDAR-30B-A3B-Chat (MoE model)
Adapted from sdar_fsdp_sft_trainer.py with MoE-specific modifications.

SDAR-30B-A3B-Chat Model Configuration:
- Architecture: MoE with 128 experts, 8 experts per token
- Model size: ~30B parameters
- mask_token_id: 151643 (same as eos_token_id)
- pad_token_id: 151643
- Uses flex_attention and logits_to_keep for loss computation
"""
...
class FSDPdLLMSFTTrainer:
    ...
    Key differences from SDAR trainer:
    - MoE model with 128 experts
    - Larger model size (~30B parameters)
    - Uses flex_attention with logits_to_keep for loss computation
    - Size-based FSDP wrapping to avoid sharding expert weights
    """
    ...
    def _build_model_optimizer(self):
        ...
        is_moe = (hasattr(config, "num_experts") and config.num_experts > 1) or \
                 (hasattr(config, "n_routed_experts") and config.n_routed_experts > 1)
        if self.device_mesh.get_rank() == 0:
            print(f"SDAR model detected: MoE={is_moe}")
            if is_moe:
                print(f"  - num_experts: {getattr(config, 'num_experts', 'N/A')}")
                print(f"  - num_experts_per_tok: {getattr(config, 'num_experts_per_tok', 'N/A')}")
            print(f"  - block_size: {getattr(config, 'block_size', 'N/A')}")

        if self.config.ulysses_sequence_parallel_size > 1:
            assert self.use_remove_padding, "Sequence parallel is only supported when remove_padding is enabled"

        # This may be very large for 30B model
    ...
    def _compute_loss_and_backward(self, batch, do_backward=True):
        """Compute loss with flex_attention and logits_to_keep for SDAR models"""
        ...
        with context, torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            if not use_sp:
                ...
                # SDAR uses flex_attention with logits_to_keep
                ...
                if input_ids.size(0) != 1:
                    raise ValueError("SDAR-MoE sequence parallelism currently requires micro_batch_size_per_gpu == 1.")

                sp_size = get_ulysses_sequence_parallel_world_size()
                dp_group = self.ulysses_device_mesh["dp"].get_group()

                shift_labels = torch.roll(labels, shifts=-1, dims=1)
                shift_loss_mask = shift_labels.ne(-100)
                shift_loss_mask[:, -1] = False

                input_ids_sliced, position_ids_sliced, _ = ulysses_pad_and_slice_inputs(
                    input_ids,
                    position_ids,
                    sp_size=sp_size,
                )
                attention_mask_sliced, _, _ = ulysses_pad_and_slice_inputs(
                    attention_mask,
                    None,
                    sp_size=sp_size,
                )
                shift_labels_sliced, _, _ = ulysses_pad_and_slice_inputs(
                    shift_labels,
                    None,
                    sp_size=sp_size,
                )
                shift_loss_mask_sliced, _, _ = ulysses_pad_and_slice_inputs(
                    shift_loss_mask.to(dtype=torch.long),
                    None,
                    sp_size=sp_size,
                )
                shift_loss_mask_sliced = shift_loss_mask_sliced.bool()

                output = self.fsdp_model(
                    input_ids=input_ids_sliced,
                    attention_mask=attention_mask_sliced.bool(),
                ...
                local_targets = shift_labels_sliced.squeeze(0)
                local_loss_mask = shift_loss_mask_sliced.squeeze(0)
                local_loss = loss_fct(logits, local_targets)
                local_loss = local_loss * local_loss_mask.to(local_loss.dtype)

                valid_token_this_rank = shift_loss_mask.sum().to(dtype=torch.float32)
                ...
                loss = local_loss.sum() / (valid_token_this_rank + 1e-8) * dp_size
