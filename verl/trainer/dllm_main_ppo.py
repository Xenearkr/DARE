...
from verl.trainer.ppo.dllm_ray_trainer import DLLMRayPPOTrainer
from verl.trainer.ppo.dllm_reward import load_reward_manager
...
class TaskRunner:  # Encapsulate main training logic
    def run(self, config):  # Main execution method
        ...
        if config.reward_model.enable:  # Check if the reward model is enabled.
        ...
        if config.algorithm.use_kl_in_reward or config.actor_rollout_ref.actor.use_kl_loss:
            role_worker_mapping[Role.RefPolicy] = ray.remote(DLLMActorRolloutRefWorker)
